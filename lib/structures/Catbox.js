...
const { isURL, filenameFromBuffer, Resolvers: { resolveFiles, resolveShort } } = require('../utils')
...
module.exports = class Catbox extends Request {
  ...
  /**
   * Upload as buffer
   * @param {object} object A buffer with file data to upload
   * @returns {Promise<string>}
   */
  uploadBuffer (buffer) {
    if (typeof buffer !== 'object') {
      throw new TypeError('UPLOAD_INVALID_PARAMETER', 'buffer', 'a buffer')
    }

    return filenameFromBuffer(buffer).then((filename) => this._request(
      {
        reqtype: 'fileupload',
        fileToUpload: buffer
      },
  ...
  /**
   * Upload by file name
   * @param {string} url A path to the file
   * @returns {Promise<string>}
   */
  uploadFile (path) {
    ...
    const _isURL = isURL(path);

    if (_isURL.is) {
      throw new TypeError("UPLOAD_INVALID_PARAMETER", "path", "a Path");
    }
  ...
  uploadURL (url) {
    if (typeof url !== 'string') {
      throw new TypeError('UPLOAD_INVALID_PARAMETER', 'url', 'a string')
    }

    const _isURL = isURL(url);

    if (!_isURL.is) {
      throw new TypeError("UPLOAD_INVALID_PARAMETER", "url", "a URL");
    }
